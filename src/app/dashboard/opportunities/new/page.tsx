...
export default async function NewOpportunityPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="mx-auto max-w-[1100px]">
    <header className="flex flex-col gap-6 border-b border-[var(--line)] pb-7 md:flex-row md:items-end md:justify-between">
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--accent)]">Commercial intake</p><h1 className="mt-3 text-4xl font-medium tracking-[-0.045em] md:text-5xl">Create opportunity</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-[var(--muted)]">Capture only the information needed to qualify demand and begin customer readiness.</p></div>
    ...
    {!ready ? <section className="mt-7 border border-[var(--line)] p-7"><p className="text-sm font-semibold">Customer and site required</p><p className="mt-3 text-sm text-[var(--muted)]">Create a customer and linked site before opening an opportunity.</p></section> :
    <form action={createOpportunity} className="mt-7 space-y-7">
      <section className="border border-[var(--line)]"><div className="border-b border-[var(--line)] p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">01 · Context</p><h2 className="mt-2 text-2xl font-medium">Opportunity definition</h2></div>
        <div className="grid gap-5 p-5 md:grid-cols-2 md:p-6">
          ...
          <label className="text-xs font-semibold">Customer<select required name="customer_id" className="mt-2 min-h-11 w-full border border-[var(--line)] bg-[var(--background)] px-3 text-sm font-normal"><option value="">Select customer</option>{customers?.map(c => <option key={c.id} value={c.id}>{c.display_name || c.name}</option>)}</select></label>
          <label className="text-xs font-semibold">Site<select required name="site_id" className="mt-2 min-h-11 w-full border border-[var(--line)] bg-[var(--background)] px-3 text-sm font-normal"><option value="">Select site</option>{sites?.map(s => <option key={s.id} value={s.id}>{s.name}{s.postcode ? ` · ${s.postcode}` : ""}</option>)}</select></label>
