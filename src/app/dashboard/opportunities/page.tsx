...
export default async function OpportunitiesPage() {
  ...
  const { data: opportunities } = await supabase.from("opportunities").select("id,title,reference,stage,estimated_value_gbp,created_at,customers(name),sites(name),profiles(full_name)").order("created_at", { ascending: false });
  ...
  return <div className="mx-auto max-w-[1500px]">
    <header className="flex flex-col gap-6 border-b border-[var(--line)] pb-7 md:flex-row md:items-end md:justify-between">
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--accent)]">Commercial pipeline</p><h1 className="mt-3 text-4xl font-medium tracking-[-0.045em] md:text-5xl">Opportunities</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-[var(--muted)]">Move qualified demand through readiness evidence and an indicative commercial proposal.</p></div>
