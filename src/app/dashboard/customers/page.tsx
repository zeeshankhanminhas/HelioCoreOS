...
export default function CustomersPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: customers, error: customerError }, { data: sites }, { data: projects }] = await Promise.all([
        client.from("customers").select("id, name, display_name, customer_kind, customer_category, status, country_code, contact_name, contact_email, phone, created_at").order("created_at", { ascending: false }),
        client.from("sites").select("id, customer_id"),
        client.from("projects").select("id, customer_id, status"),
      ]);

      if (cancelled) return;
      if (customerError) {
        setError(customerError.message || "The customer register could not be loaded.");
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-[1500px]" data-testid="customers-neon-register">
      <PageHeader
